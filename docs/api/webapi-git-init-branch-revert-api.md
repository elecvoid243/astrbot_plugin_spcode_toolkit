...
在已存在空目录上执行 `git init`,**不检查**项目加载(因为"未加载" = "你要 new")。

**Body (JSON, 必传)**:
```json
{
  "path": "D:/Users/dev/projects/myapp",     // 必填,必须存在且为空目录(除非 bare)
...
```json
{
  ...
  "data": {
    ...
    "initial_branch": "main",
    "bare": false,
    "hint": "init: created empty Git repository"
  }
}
```
...
| `directory_not_empty` | 目录有文件(且 non-bare) |
| `already_a_git_repo` | 目录已是 git repo(.git 已存在) |
...
| | `directory_not_empty` | 目录非空(且 non-bare) |
