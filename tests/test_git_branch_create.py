...
def test_create_name_invalid_chars(loaded_umo):
    plugin = _make_plugin()
    result = _run(
        git_branch_create.handle(plugin, umo=loaded_umo, body={"name": "bad..name"})
    )
...
def test_create_name_is_HEAD(loaded_umo):
    plugin = _make_plugin()
    result = _run(
        git_branch_create.handle(plugin, umo=loaded_umo, body={"name": "HEAD"})
    )
...
def test_create_default_from_HEAD(loaded_umo, existing_repo):
    plugin = _make_plugin()
    result = _run(
        git_branch_create.handle(plugin, umo=loaded_umo, body={"name": "new-branch"})
    )
...
def test_create_force_overwrites(loaded_umo, existing_repo):
    ...
    plugin = _make_plugin()
    r1 = _run(
        git_branch_create.handle(plugin, umo=loaded_umo, body={"name": "x-branch"})
    )
...
def test_create_branch_exists(loaded_umo, existing_repo):
    ...
    plugin = _make_plugin()
    result = _run(
        git_branch_create.handle(plugin, umo=loaded_umo, body={"name": "feature/x"})
    )
