...
v2.21.0 (2026-07-21, elecvoid243): 新增请求体 `scope` 字段
  (file-discard-hunk-api v2.21)。当客户端明确传入 `unstaged` / `staged` 时
  直接决定 `git apply --reverse[ --cached]` 路径,跳过 porcelain X/Y
  auto-detect —— 修复 MM 状态(staged + worktree 同时 dirty)下用户看
  unstaged 视图点 discard 时反向应用到 index 的反向操作 bug。`all` 视图
  / 缺失 / 无效值仍走原 auto-detect 路径,向后兼容旧客户端。
"""
    ...
    **kwargs: Any,
    ...
    if patch_meta.err is not None:
        return _make_file_discard_hunk_empty_envelope(
            ...
            directory=directory,
            patch_sha=patch_sha,
            reason=patch_meta.err,
            elapsed_ms=_elapsed(),
        )

    # Step 12: scope 选择
    # v2.21 (2026-07-21, elecvoid243): 优先用客户端传入的 scope
    # (file-discard-hunk-api v2.21)。这样在 MM 状态(staged + worktree
    # 同时有差异),前端知道用户在看哪个视图,直接走对应的
    # `git apply --reverse[ --cached]` 路径,不再依赖 porcelain X/Y
    # 模糊判断 —— 旧逻辑在 MM 状态固定走 --cached(因为 X 列有 staged
    # 标记),会把 worktree 改动反向应用到 index 上,导致用户操作反向。
    # 'all' 视图没有明确的单路径语义,仍走 auto-detect;缺失/无效值也
    # 走 auto-detect,向后兼容旧客户端(未升级 v2.21 前端)。
    raw_scope = body.get("scope")
    client_scope: str | None = raw_scope if isinstance(raw_scope, str) else None
    if client_scope not in ("unstaged", "staged", "all"):
        client_scope = None
    if client_scope in ("unstaged", "staged"):
        # 客户端明确指定路径:跳过 porcelain auto-detect。
        # 即便文件状态实际为 untracked / not modified,客户端的
        # 决定优先 —— 让上层报 NOT_MODIFIED / UNTRACKED_FILE 等更
        # 具体的失败原因,而不是默默用错路径。
        scope = client_scope
        if scope == "staged":
            apply_base = [
        ...
        # Fallback:porcelain auto-detect (v1 行为,保留以兼容旧客户端
        # 和 `all` 视图——后者没有明确的 --cached vs 无 flag 语义)。
        ...
        if not porcelain.strip():
            return _make_file_discard_hunk_empty_envelope(
                ...
                scope="unstaged",
                patch_sha=patch_sha,
                reason=ReasonCode.NOT_MODIFIED,
                elapsed_ms=_elapsed(),
            )
        ...
        if x_status == "?" and y_status == "?":
            return _make_file_discard_hunk_empty_envelope(
                ...
                scope="unstaged",
                patch_sha=patch_sha,
                reason=ReasonCode.UNTRACKED_FILE,
                stderr=porcelain,
                elapsed_ms=_elapsed(),
            )
    ...
    # 注入 --check
    check_args = (
        check_args[: check_args.index("apply") + 1]
        + ["--check"]
        + check_args[check_args.index("apply") + 1 :]
    )
