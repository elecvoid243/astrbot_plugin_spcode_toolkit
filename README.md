...
| `todo_create` / `todo_query` / `todo_modify` / `todo_clear` (4 个) | 任务管理 | v2.6+ LLM 自我管理 todo list，拆分为 4 个独立工具；旧 `todo_list` 已转 stub |
...
| `todo_create` / `todo_query` / `todo_modify` / `todo_clear` (4 个) | LLM 自我管理任务清单（v2.6+ 拆分 API） |
...
- `todo_list` (v2.6.1+) — 勾选后自动展开为 `todo_create`、`_query`、`_modify`、`_clear` 共 4 个工具（4 个强相关，缺一不可）

> 旧版（v2.6.0 之前）配置中可能含独立的 `todo_create` / `todo_query` / `todo_modify` / `todo_clear` 条目，升级后仍可正常工作。v2.6.1 起配置 UI 只暴露 `todo_list` 组别名，引导用户使用推荐方式。
...
### todo_create / todo_query / todo_modify / todo_clear — LLM 自我管理任务清单（v2.6+ 拆分 API）

跨会话持久化的任务清单系统，**v2.6 起拆分为 4 个独立 LLM 工具**，每个工具职责单一、参数最小化。

**4 工具分工：**

- `todo_create(items?, title?, from_path?)` — 创建/覆盖当前用户的列表（v2.9+ 新增 `from_path` 从 .md 领养）
- `todo_query()` — 读取当前列表（含 `stats` + `attention_items`）
- `todo_modify(mode='add'|'update'|'delete', ...)` — 统一修改入口
- `todo_clear()` — 删除整个列表
...
- 领养（adopt）语义：保留文件中的 `status` / `notes`，但**重排 ID 从 1**、**改写 `sender_key` 为当前用户**、`updated_at = now()`
...
**notes 三态**（仅 `todo_modify(mode='update')`）：None=保留旧值 / `""`=清空 / `"xxx"`=覆盖。
...
# 3. 批量追加
todo_modify(mode="add", items=[
    {"title": "写测试", "status": "in_progress"},
    {"title": "补文档", "status": "pending"},
])

# 4. 更新状态（item_ids 可为单个 int 或 list[int]）
todo_modify(mode="update", item_ids=[1, 3], status="done")
todo_modify(mode="update", item_ids=2, status="in_progress",
            notes="等待用户确认函数切分边界")

# 5. 删除单个 / 批量
todo_modify(mode="delete", item_ids=1)
todo_modify(mode="delete", item_ids=[2, 3])
