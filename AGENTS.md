...
> **当前版本: v2.29.0** · Author: elecvoid243 · 最后更新: 2026-09-13
...
└── tools/                        # 工具实现层（子包化，PR-0~PR-7 拆分自 main.py）
    ...
    └── webapi/                   # Web API 层（44 条路由记录 / 42 个唯一路径，每端点一文件）
        ...
        ├── git_stash.py          #   GET/POST /spcode/git-stash + POST git-stash-pop/-drop (2026-08-21, 列表+文件明细 / push -u / pop / drop)
...
13. **版本号统一**：当前版本统一为 **v2.29.0**。发布时同步更新 `metadata.yaml` 的 `version` 字段
